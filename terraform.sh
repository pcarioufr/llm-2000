...
generate_env_files() {
    ...
    } > .env/datadog-synthetics-worker.env
    ...
    # Generate datadog-op-worker.env
    {
        echo "# Observability Pipelines Worker Configuration"
        echo "# Generated by terraform.sh from Terraform outputs"
        echo ""
        echo "DD_OP_PIPELINE_ID='$(echo "$OUTPUTS" | jq -r '.op_pipeline_id.value')'"
        echo ""
        echo "# Enable Remote Configuration to fetch pipeline config from Datadog"
        echo "DD_OP_REMOTE_CONFIGURATION_ENABLED=true"
    } > .env/datadog-op-worker.env
}

# Main script logic
COMMAND="$1"
shift  # Remove the first argument (command), leaving any additional flags
